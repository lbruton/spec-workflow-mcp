...
export async function findAvailablePort(host: string): Promise<number> {
  ...
  throw new Error(
    `No available ephemeral port found in range ${ephemeralStart}-${ephemeralEnd} on ${host}`,
  );
}
 ...
 */
export async function validateAndCheckPort(
  port: number,
  host: string,
  skipDashboardCheck: boolean = false,
): Promise<void> {
  ...
  if (!skipDashboardCheck) {
    ...
    if (isDashboard) {
      ...
      throw new Error(
        `Port ${port} is already in use on ${host} by an existing dashboard instance.`,
      );
    }
  }
  ...
  if (!available) {
    throw new Error(
      `Port ${port} is already in use on ${host}. Please choose a different port or omit --port to use an ephemeral port.`,
    );
  }
}
