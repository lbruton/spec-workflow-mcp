    ...
    <>
      ...
      >
        ...
        <div className="hidden lg:flex items-center justify-between p-4 border-b border-[var(--border-default)]">
          {!isCollapsed && (
            <h2 className="text-lg font-semibold text-[var(--text-primary)]">SpecFlow</h2>
          )}
          <button
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-1">
          {navigationSections.map((section, sectionIdx) => (
            <React.Fragment key={sectionIdx}>
              ...
              {section.labelKey && (
                <div
                  className={`pt-3 ${sectionIdx > 0 ? 'mt-2 border-t border-[var(--border-default)]' : ''}`}
                >
                  ...
                  <button
                    ...
                    onClick={() =>
                      window.open(
                        item.href,
                        '_blank',
                        'toolbar=no,location=no,menubar=no,status=no,scrollbars=yes,resizable=yes,width=1400,height=900',
                      )
                    }
                    className={`
