...
export function useSoundNotifications(options: SoundNotificationOptions = {}) {
  ...
  const getSoundUri = useCallback(
    (sound: NotificationSound): string | null => {
  ...
  const getAudioElement = useCallback(
    (sound: NotificationSound): HTMLAudioElement | null => {
      if (!enabled) {
        return null;
      }
  ...
  const playNotification = useCallback(
    async (sound: NotificationSound): Promise<void> => {
      console.log('playNotification called with:', {
        sound,
        enabled,
      ...
      if (!userHasInteracted.current) {
        console.warn(
          'Cannot play sound - user has not interacted with page yet (browser autoplay restriction)',
        );
        return;
      }
